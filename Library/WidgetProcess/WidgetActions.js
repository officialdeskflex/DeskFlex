...
const widgetUtils = {
  ...
  execute: (cmd) => {
    exec(
      cmd,
      (error) => error && console.error(`Exec "${cmd}" failed:`, error)
    );
  },
  ...
  draggable: ([val, widgetName], currentWidgetName) => {
    if (!["0", "1"].includes(String(val))) {
      log(
        `Invalid draggable value: ${val}. Expected "0" or "1".`,
        "error",
        currentWidgetName || currentWidgetName
      );
    } else {
      ipcRenderer.send(
        "widget-set-draggable",
        val,
  ...
  keeponscreen: ([val, widgetName], currentWidgetName) => {
    if (!["0", "1"].includes(String(val))) {
      log(
        `Invalid keepOnScreen value: ${val}. Expected "0" or "1".`,
        "error",
        widgetName || currentWidgetName
      );
    } else {
      ipcRenderer.send(
        "widget-set-keep-on-screen",
        val,
        widgetName || currentWidgetName
      );
    }
  ...
  clickthrough: ([val, widgetName], currentWidgetName) => {
    if (!["0", "1"].includes(String(val))) {
      log(
        `Invalid clickThrough value: ${val}. Expected "0" or "1".`,
        "error",
        widgetName || currentWidgetName
      );
    } else {
...
function wireWidgetEvents() {
  document.querySelectorAll(".widget").forEach((widgetElement) => {
    ...
    widgetElement.addEventListener("mouseover", () =>
      runAction(widgetElement, "mouseoveraction")
    );
    widgetElement.addEventListener("mouseleave", () =>
      runAction(widgetElement, "mouseleaveaction")
    );
